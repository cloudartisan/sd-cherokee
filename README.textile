...
The Cherokee plugin will default to checking
@http://localhost/about/info/py?type=connection_details@ for stats.  For most
people, that will probably be sufficient.  However, if you're running Cherokee
on a different server or using a different path you'll want the plugin to use
a different URL to get stats.
